...
## Tech Stack
- **Next.js 15 (App Router) + TypeScript**, statically exported (SSG)
- Pages under `app/`; shared UI under `components/`; global styles in `styles/globals.scss`
- Static assets in `public/` (images, flyers, PDFs)
- Sitemap generated by `app/sitemap.ts` (TypeScript route handler)
- Redirects in `vercel.json` (`redirects` array — e.g. orphan PDFs → HTML twin pages)
- Vercel deployment, region `gru1` (São Paulo)
- Legacy `articulos/`, `servicios/`, `css/`, `js/`, `index.html` at repo root are static-export artifacts — **do not edit** them; edit under `app/` instead

## Commands
```bash
pnpm install            # Install dependencies
pnpm run build          # Next.js production build + static export to out/
gitpush.sh "<message>"  # ONLY sanctioned push path; runs pre-push hook (lint/typecheck/build) and deploys to Vercel
```

Do not invoke `git push` directly. Do not invoke a dev server unless explicitly asked.
...
### Service Pages (SEO Landing Pages) — `app/servicios/<slug>/page.tsx`
- `/servicios/divorcios-buenos-aires`
- `/servicios/derecho-familia`
- `/servicios/derecho-familia-internacional`
...
### Article Pages — `app/articulos/<slug>/page.tsx`
20+ articles (academic + SEO-targeted hub content). Authoritative list lives in `app/sitemap.ts`.
...
**All new clickable elements MUST have tracking.** Use the `<TrackedLink>` component (`components/TrackedLink.tsx`) which routes events to GA4 + GTM. Equivalent data attributes:
...
## Key Files
- `app/page.tsx` - Homepage (Hero, About, Specialties, Comunicaciones, Testimonials, Publications, Contact) with all schema markup
- `app/sitemap.ts` - Sitemap source of truth (add new slugs to the `services` / `articles` arrays here)
- `app/servicios/<slug>/page.tsx` - Service landing pages
- `app/articulos/<slug>/page.tsx` - Article pages
- `components/` - Shared UI (`TrackedLink`, `ServiceCTA`, `Breadcrumb`, `JsonLd`, `ServiceNav`)
- `vercel.json` - Region pin (`gru1`) + 301 redirects
